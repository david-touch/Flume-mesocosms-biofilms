...
#16S 2021/2022 data preparation
...
#Divide per Year and filter for more than 1 read in at least 2 samples
Sentinel16S_2021_bacteria <- Sentinel16S_bacteria %>% 
  ps_filter(Year == 2021)
Sentinel16S_2021_bacteria
Sentinel16S_2021_bacteria_filt <- Sentinel16S_2021_bacteria %>% 
  filter_taxa(function(x) sum(x > 1) > (0.004*length(x)), TRUE)
Sentinel16S_2021_bacteria_filt

saveRDS(Sentinel16S_2021_bacteria, file = "Sentinel16S_2021_bacteria.rds", ascii = FALSE, version = NULL,
        compress = TRUE, refhook = NULL)
saveRDS(Sentinel16S_2021_bacteria_filt, file = "Sentinel16S_2021_bacteria_filt.rds", ascii = FALSE, version = NULL,
        compress = TRUE, refhook = NULL)
...
#2021
sample_data(Sentinel16S_2021_bacteria_filt)$Sample <- as.factor(sample_data(Sentinel16S_2021_bacteria_filt)$Sample)
sample_data(Sentinel16S_2021_bacteria_filt)$Treatment <- as.factor(sample_data(Sentinel16S_2021_bacteria_filt)$Treatment)
sample_data(Sentinel16S_2021_bacteria_filt)$Flow <- as.factor(sample_data(Sentinel16S_2021_bacteria_filt)$Flow)
sample_data(Sentinel16S_2021_bacteria_filt)$Temperature <- as.factor(sample_data(Sentinel16S_2021_bacteria_filt)$Temperature)
sample_data(Sentinel16S_2021_bacteria_filt)$Drought <- as.factor(sample_data(Sentinel16S_2021_bacteria_filt)$Drought)

Sentinel16S_2021_filt_merged <- merge_samples(Sentinel16S_2021_bacteria_filt, "Sample", fun = "mean") %>% 
...
otu_table(Sentinel16S_2021_filt_merged) <- t(otu_table(Sentinel16S_2021_filt_merged))
Sentinel16S_2021_filt_merged

saveRDS(Sentinel16S_2021_filt_merged, file = "Sentinel16S_2021_filt_merged.rds", ascii = FALSE, version = NULL,
        compress = TRUE, refhook = NULL)
...
Sentinel16S_2021_water_bacteria <- Sentinel16S_water_bacteria %>% ps_filter(Year == 2021)
Sentinel16S_2021_water_bacteria
...
FigureS4 <- ggarrange(MicEco_16S, MicEco_16S_abundance, ncol = 1, nrow = 2, labels = c("A", "B"), font.label = list(size = 18))
FigureS4
ggsave("FigureS4.pdf", FigureS4, width = 20,
       height = 20,
       units = "cm")
